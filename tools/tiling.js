...
async function generateTiles() {
    ...
    // Tiles sind 513x513: letzter Pixel = erster Pixel des Nachbar-Tiles
    // → Tile A bei UV u=1.0 sampelt selben Quell-Pixel wie Tile B bei u=0.0 → keine Naht
    const TW = TILE_SIZE + 1;

    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
            const tile = new Uint16Array(TW * TW);
            for (let ty = 0; ty < TW; ty++) {
                for (let tx = 0; tx < TW; tx++) {
                    const sx = Math.min(x * TILE_SIZE + tx, FULL_SIZE - 1);
                    const sy = Math.min(y * TILE_SIZE + ty, FULL_SIZE - 1);
                    tile[ty * TW + tx] = raw16[sy * FULL_SIZE + sx];
                }
            }
    ...
    console.log('Fertig! 513x513 Tiles (nahtlos) gespeichert.');
}
